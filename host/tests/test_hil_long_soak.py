...
def pytest_addoption(parser):
    ...
    # Long soak defaults (5 min for CI, use --soak-seconds=1800 for full 30 min test)
...
async def test_hil_long_soak_link_stability(request):
    ...
    Default duration: 300s (5 min) for CI. Use --soak-seconds=1800 for full 30 min test.
    Skip in quick test runs with: pytest -m "not slow"
    """
    ...
    # Default changed from 1800 to 300 for CI
    soak_s = _get_float_opt(request, "--soak-seconds", "SOAK_SECONDS", 300.0)
