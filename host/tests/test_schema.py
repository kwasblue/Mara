...
class TestCommandsDomainFiles:
    ...
    def test_commands_has_expected_count(self):
        ...
        from mara_host.tools.schema import COMMANDS
        # Total should be 121 commands across all domains (including WiFi, Benchmark commands)
        # Added: CMD_SET_SIGNING_KEY, CMD_RELEASE_SESSION (safety), CMD_CTRL_GRAPH_DEBUG, CMD_CTRL_GRAPH_COMMIT (control)
        # Added: CMD_CTRL_SIGNAL_TRACE, CMD_CTRL_AUTO_SIGNALS_CONFIG (auto-signals support)
        assert len(COMMANDS) == 121, f"Expected 121 commands, got {len(COMMANDS)}"
    ...
    def test_individual_domain_exports(self):
        ...
        # CONTROL: +2 for CMD_CTRL_SIGNAL_TRACE, CMD_CTRL_AUTO_SIGNALS_CONFIG
        assert len(CONTROL_COMMANDS) == 23, f"CONTROL: expected 23, got {len(CONTROL_COMMANDS)}"
        assert len(CONTROL_COMMAND_OBJECTS) == 23, f"CONTROL_OBJECTS: expected 23, got {len(CONTROL_COMMAND_OBJECTS)}"
        ...
        assert len(BENCHMARK_COMMANDS) == 7, f"BENCHMARK: expected 7, got {len(BENCHMARK_COMMANDS)}"
