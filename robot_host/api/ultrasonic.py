# robot_host/api/ultrasonic.py
"""
Ultrasonic distance sensor interface.

Example:
    from robot_host import Robot, Ultrasonic
...
class Ultrasonic:
    ...
    def __init__(
        ...
        self._ok = False
        self._ts_ms: int = 0
        self._callbacks: list[Callable[[UltrasonicReading], None]] = []

        # Subscribe to telemetry
        robot.on("telemetry.ultrasonic", self._on_telemetry)
    ...
    def _on_telemetry(self, data: dict) -> None:
        """Handle incoming ultrasonic telemetry."""
        # Filter by sensor_id if present
        data_sensor_id = data.get("sensor_id", self.sensor_id)
        if data_sensor_id != self.sensor_id:
            return
    ...
    async def attach(self) -> None:
        ...
        await self.client.cmd_ultrasonic_attach(sensor_id=self.sensor_id)
        self._attached = True

    async def read(self) -> None:
        """Trigger a new distance reading."""
        await self.client.cmd_ultrasonic_read(sensor_id=self.sensor_id)

    def on_update(self, callback: Callable[[UltrasonicReading], None]) -> None:
        """
        Register callback for distance updates.

        Args:
            callback: Function called with UltrasonicReading on each update
        """
    ...
    def __repr__(self) -> str:
        ...
        if self._ok:
            return f"Ultrasonic(id={self.sensor_id}, {status}, {self._distance_cm:.1f}cm)"
        return f"Ultrasonic(id={self.sensor_id}, {status}, no reading)"
