# robot_host/module/camera_module.py
"""
...
class CaptureMode(Enum):
    """Frame capture mode."""
    POLLING = "polling"    # HTTP polling /jpg endpoint
    STREAMING = "streaming"  # MJPEG stream /stream endpoint
...
class CameraModule:
    ...
    def start(self) -> None:
        ...
        print(f"[CameraModule:{self.name}] Started in {self.mode.value} mode")
    ...
    def _show_frame(self, display_frame: np.ndarray) -> None:
        ...
        if self.show_stats:
            ...
            text = f"FPS: {stats.avg_fps:.1f} | Lat: {stats.avg_latency_ms:.0f}ms | {self.mode.value}"
            cv2.putText(
                frame, text, (10, 25),
