#!/usr/bin/env python3
"""
ESP32 Pin Management CLI

Usage:
    python pins.py pinout            # Visual board pinout diagram
    python pins.py list              # Show all pins with status
    python pins.py free              # Show available pins
    python pins.py info 23           # Show details for GPIO 23
    python pins.py assign NAME PIN   # Assign a pin (updates pins.json)
    python pins.py remove NAME       # Remove a pin assignment
    python pins.py suggest pwm       # Suggest best pins for PWM use
    python pins.py validate          # Check current assignments for issues
"""
...
def print_pin_table(pins: list[int], assignments: dict[int, str], show_all: bool = False) -> None:
    """Print a formatted table of pins."""
    print(f"{'GPIO':<5} {'Status':<18} {'Capabilities':<24} {'Notes'}")
    print("-" * 85)
    ...
    for gpio in pins:
        ...
        if gpio in assignments:
            status = f"[{assignments[gpio]}]"
        elif gpio in FLASH_PINS:
            status = "(FLASH - unusable)"
        elif info.warning:
            status = "(caution)"
        else:
            status = "available"
        ...
        # Notes (truncated)
        notes = info.notes[:30] + "..." if len(info.notes) > 33 else info.notes

        print(f"{gpio:<5} {status:<18} {caps:<24} {notes}")
...
def cmd_list(args: list[str]) -> None:
    ...
    print("\n=== ESP32 GPIO Pin Status ===\n")
    print_pin_table(all_gpios, assignments)

    # Summary
    used = len(assignments)
    usable = len([g for g in ESP32_PINS if g not in FLASH_PINS])
    print(f"\nAssigned: {used}/{usable} usable pins")
    print(f"Safe pins available: {len(SAFE_PINS - set(assignments.values()))}")
...
def cmd_free(args: list[str]) -> None:
    ...
    print("RECOMMENDED (no boot/flash restrictions):")
    safe_free = sorted(SAFE_PINS - assigned_gpios)
    print_pin_table(safe_free, assignments)

    # Input-only pins
    print("\nINPUT-ONLY PINS (GPIO 34-39, no output/PWM):")
    input_free = sorted(INPUT_ONLY_PINS - assigned_gpios)
    print_pin_table(input_free, assignments)

    # Boot pins (usable with caution)
    print("\nBOOT PINS (usable but check warnings):")
    boot_free = sorted((BOOT_PINS - assigned_gpios) - FLASH_PINS)
    print_pin_table(boot_free, assignments)


def cmd_info(args: list[str]) -> None:
    """Show detailed info for a specific pin."""
    if not args:
        print("Usage: pins.py info <gpio_number>")
        return

    try:
        gpio = int(args[0])
    except ValueError:
        print(f"Invalid GPIO number: {args[0]}")
        return

    if gpio not in ESP32_PINS:
        print(f"GPIO {gpio} not found in ESP32 pinout")
        return
    ...
    if gpio in assignments:
        print(f"Status:       ASSIGNED as '{assignments[gpio]}'")
    elif gpio in FLASH_PINS:
        print(f"Status:       UNUSABLE (connected to flash)")
    else:
        print(f"Status:       Available")

    print(f"Capabilities: {cap_str(info.capabilities)}")
    print(f"Notes:        {info.notes}")

    if info.adc_channel:
        print(f"ADC:          {info.adc_channel}")
    if info.touch_channel is not None:
        print(f"Touch:        Touch{info.touch_channel}")
    if info.rtc_gpio is not None:
        print(f"RTC GPIO:     {info.rtc_gpio}")

    if info.warning:
        print(f"\n⚠️  WARNING: {info.warning}")

    if gpio in SAFE_PINS:
        print("\n✓ This is a safe, recommended pin for general use.")


def cmd_assign(args: list[str]) -> None:
    """Assign a name to a GPIO pin."""
    if len(args) < 2:
        print("Usage: pins.py assign <NAME> <gpio_number>")
        print("Example: pins.py assign MOTOR_RIGHT_PWM 15")
        return
    ...
    try:
        ...
        print(f"Invalid GPIO number: {args[1]}")
        return

    # Validate
    if gpio not in ESP32_PINS:
        print(f"Error: GPIO {gpio} not found in ESP32 pinout")
        return

    if gpio in FLASH_PINS:
        print(f"Error: GPIO {gpio} is connected to flash and cannot be used")
        return
    ...
    if name in pins:
        print(f"Error: Name '{name}' already assigned to GPIO {pins[name]}")
        return

    # Check if GPIO already assigned
    if gpio in assignments:
        print(f"Error: GPIO {gpio} already assigned as '{assignments[gpio]}'")
        return
    ...
    pins[name] = gpio
    save_pins(pins)
    print(f"✓ Assigned {name} = GPIO {gpio}")
    print(f"\nRun 'python -m robot_host.tools.generate_all' to regenerate code.")


def cmd_remove(args: list[str]) -> None:
    """Remove a pin assignment."""
    if not args:
        print("Usage: pins.py remove <NAME>")
        return
    ...
    if name not in pins:
        print(f"Error: '{name}' not found in pin assignments")
        return

    gpio = pins.pop(name)
    save_pins(pins)
    print(f"✓ Removed {name} (was GPIO {gpio})")
    print(f"\nRun 'python -m robot_host.tools.generate_all' to regenerate code.")
...
def cmd_suggest(args: list[str]) -> None:
    ...
    if not args:
        print("Usage: pins.py suggest <use_case>")
        print("Use cases: pwm, adc, input, output, i2c, spi, uart, touch, dac")
        return

    use_case = args[0].lower()
    assignments = get_assignments()
    assigned_gpios = set(assignments.keys())
    ...
    if use_case not in use_map:
        print(f"Unknown use case: {use_case}")
        print("Valid: " + ", ".join(use_map.keys()))
        return

    required = use_map[use_case]

    print(f"\n=== Best pins for {use_case.upper()} ===\n")
    ...
    if not candidates:
        print("No available pins match this use case.")
        return

    gpios = [g for _, g in candidates[:10]]
    print_pin_table(gpios, assignments)

    # Special notes
    if use_case == "adc":
        print("\nNote: ADC2 pins (GPIO 0,2,4,12-15,25-27) don't work when WiFi is active.")
        print("      ADC1 pins (GPIO 32-39) work with WiFi.")
    elif use_case == "i2c":
        print("\nNote: Default I2C pins are GPIO 21 (SDA) and 22 (SCL).")
...
def cmd_validate(args: list[str]) -> None:
    ...
    for name, gpio in pins.items():
        if gpio not in ESP32_PINS:
            issues.append(f"{name} (GPIO {gpio}): Invalid GPIO number")
            continue

        info = ESP32_PINS[gpio]

        if gpio in FLASH_PINS:
            issues.append(f"{name} (GPIO {gpio}): Connected to flash, unusable!")

        if info.warning:
            warnings.append(f"{name} (GPIO {gpio}): {info.warning}")
    ...
    if not issues and not warnings:
        print("✓ All pin assignments look good!")

    print(f"\nTotal: {len(pins)} pins assigned")
...
def cmd_pinout(args: list[str]) -> None:
    """Display visual ASCII pinout diagram of ESP32 DevKit."""
    output = generate_pinout()
    print(output)

    # Save to file
    pinout_file = PINS_JSON.parent / "pinout.txt"
    pinout_file.write_text(output)
    print(f"  Saved to: {pinout_file}")
    print()
